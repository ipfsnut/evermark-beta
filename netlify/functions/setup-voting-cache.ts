...
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    if (event.httpMethod === 'POST') {
      console.log('🔄 Setting up voting cache tables...');

      // Execute the migration SQL
      await supabase.rpc('exec_sql', { sql: VOTING_CACHE_SQL });
      console.log('✅ Created tables');

      await supabase.rpc('exec_sql', { sql: INDEXES_SQL });
      console.log('✅ Created indexes');

      await supabase.rpc('exec_sql', { sql: TRIGGERS_SQL });
      console.log('✅ Created triggers');

      await supabase.rpc('exec_sql', { sql: COMMENTS_SQL });
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: true, 
          message: 'Voting cache tables created successfully',
          tables: ['voting_cache', 'user_votes_cache', 'voting_cycles_cache']
        }),
      };
    }
    ...
    if (event.httpMethod === 'GET') {
      ...
      const { data: tables, error } = await supabase
        ...
        .in('table_name', ['voting_cache', 'user_votes_cache', 'voting_cycles_cache']);
      ...
      const requiredTables = ['voting_cache', 'user_votes_cache', 'voting_cycles_cache'];
