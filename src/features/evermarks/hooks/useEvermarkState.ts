  ...
  fetchEvermarks: async (options: EvermarkFeedOptions): Promise<EvermarkFeedResult> => {
    try {
      ...
      const transformedEvermarks = (data.evermarks ?? []).map((item: unknown) => {
        ...
        return {
          ...evermarkItem,
          id: String(evermarkItem.token_id),
          tokenId: Number(evermarkItem.token_id),
          title: title || String(evermarkItem.title || '') || 'Untitled',
          author: author || String(evermarkItem.author || '') || 'Unknown',
          creator: String(evermarkItem.owner || evermarkItem.author || '') || 'Unknown',
          description: description || String(evermarkItem.description || '') || '',
          metadataURI: String(evermarkItem.token_uri || ''),
          tags,
          verified: Boolean(evermarkItem.verified),
          creationTime: Date.parse(String(evermarkItem.created_at || new Date().toISOString())),
          ...
          createdAt: String(evermarkItem.created_at || new Date().toISOString()),
          updatedAt: String(evermarkItem.updated_at || evermarkItem.created_at || new Date().toISOString()),
